...
class CarcassonneTileDetector:
    ...
    def _load_cnn_model(self):
        ...
        for model_path in model_paths:
            if model_path.exists():
                try:
                    self.cnn_model = CarcassonneCNN(num_classes=24)
    ...
    def detect_tile(self, image_path: str) -> str:
        ...
        # Verificar si es una imagen blanca (sin loseta)
        ...
        if self.cnn_model is not None:
            tile_type = self._detect_with_cnn(image)
        else:
            tile_type = self._detect_with_traditional(image, has_shield)

        # Verificar confianza
        confidence = 0.9 if self.cnn_model else 0.7  # Mayor confianza para CNN
        if confidence < 0.5:
    ...
    def _is_blank_image(self, image: np.ndarray) -> bool:
        ...
        # Calcular porcentaje de píxeles blancos/claros
        total_pixels = gray.size
        white_pixels = np.sum(gray > 200)  # Píxeles muy claros
        white_ratio = white_pixels / total_pixels
        
        # También verificar varianza baja (imágenes uniformes)
        variance = np.var(gray)
        
        # Considerar blanco si >80% píxeles claros Y baja varianza
        return white_ratio > 0.8 and variance < 1000
    ...
    def _detect_with_cnn(self, image: np.ndarray) -> str:
        ...
        return self.idx_to_letter.get(predicted_idx, '?')
    ...
    def _detect_with_traditional(self, image: np.ndarray, has_shield: bool) -> str:
        ...
        # Comparar con cada referencia (usar primera imagen de cada lista)
        best_match = '?'
        best_score = 0.0

        for letter, img_list in self.references.items():
            if img_list:  # Verificar que la lista no esté vacía
                ref_img = img_list[0]  # Usar primera imagen
