...
class FieldDetector:
    ...
    def detect_fields(self, expand_barriers_iterations=3, min_area=50) -> Tuple[np.ndarray, int]:
        ...
        Args:
            expand_barriers_iterations: Cuánto expandir las barreras
        ...
        if min_area > 0:
            labeled_array, num_fields = self._filter_small_fields(labeled_array, num_fields, min_area)
        return labeled_array, num_fields

    def _filter_small_fields(self, labeled_array: np.ndarray, num_fields: int, min_area: int) -> Tuple[np.ndarray, int]:
        """
    ...
    def count_meeples_in_field(self, field_label: int, labeled_fields: np.ndarray, meeple_masks: Dict[str, np.ndarray], road_mask: np.ndarray = None) -> Dict[str, int]:
        """
        ...
        Args:
            ...
            road_mask: Máscara de caminos (opcional)
            ...
            if pixel_count >= 10:
                if road_mask is not None:
                    ...
                    if touches_road:
                        ...
                        continue
                labeled_meeples, num_meeples = ndimage.label(meeples_in_field, structure=np.ones((3, 3), dtype=int))
                counts[meeple_type] = num_meeples
            else:
                counts[meeple_type] = 0
        return counts

    def create_fields(self, labeled_fields: np.ndarray, num_fields: int, meeple_masks: Dict[str, np.ndarray], road_mask: np.ndarray = None) -> List[Field]:
        """
        ...
        for field_id in range(1, num_fields + 1):
            ...
            if area == 0:
                continue
            meeples = self.count_meeples_in_field(field_id, labeled_fields, meeple_masks, road_mask=road_mask)
            field = Field(id=field_id, pixels=field_pixels, meeples=meeples, area=area)
            fields.append(field)
        return fields