...
class CarcassonneTileDetector:
    ...
    def __init__(self, reference_folder: str = r"C:\Users\agust\OneDrive\Desktop\Uni\Proyecto\Proyecto-Final-Inteligencia-Artesanal\OpencvPruebas\PruebasAgu\Tercera_Versión_IA_SOLO_LOSETAS_COPIA_POR_LAS_DUDAS\referencias_organizadas"):
    ...
    def _load_references(self):
        ...
        print(f"Cargando referencias desde: {self.reference_folder} - VERSION ACTUALIZADA")

        total_images = 0
        # Cargar todas las imágenes PNG y JPG de la carpeta
        for letter in "ABCDEFGHIJKLMNOPQRSTUVXYZ":  # Sin W para agregar BLANCO después
            letter_folder = self.reference_folder / letter
            if letter_folder.exists():
                self.references[letter] = []
                # Cargar todas las imágenes PNG de la carpeta
                for img_file in letter_folder.glob("*.png"):
                    img = cv2.imread(str(img_file))
                    if img is not None:
                        img = cv2.resize(img, (200, 200))  # Tamaño estándar
                        self.references[letter].append(img)
                        total_images += 1
                        print(f"  [OK] {img_file.name} cargada para {letter}")
                    else:
                        print(f"  [ERROR] Error cargando {img_file.name}")
                # Cargar todas las imágenes JPG de la carpeta
    ...
    def detect_tile(self, image_path: str) -> str:
        ...
        # Verificar si es una imagen blanca (sin loseta) DESPUÉS del redimensionamiento
        ...
        if confidence < 0.6:  # Bajé el threshold de 0.5 a 0.6
            print(f"Loseta detectada: DESCONOCIDO (confianza baja: {confidence:.2f})")
            return "DESCONOCIDO"
    ...
    def _is_blank_image(self, image: np.ndarray) -> bool:
        ...
        # Criterios más estrictos para áreas blancas:
        # - >75% píxeles muy claros (>180)
        # - Intensidad media alta (>170)
        # - Varianza baja (<1000)
        is_blank = (white_ratio > 0.75 and
                   mean_intensity > 170 and
                   variance < 1000)
    ...
    def _detect_with_traditional(self, image: np.ndarray, has_shield: bool) -> str:
        ...
        # Comparar con cada referencia (usar todas las imágenes de cada lista)
        best_match = '?'
        best_score = 0.0

        for letter, img_list in self.references.items():
            if img_list:  # Verificar que la lista no esté vacía
                # Comparar con todas las imágenes de referencia para esta letra
                letter_best_score = 0.0
                for ref_img in img_list:
                    ...
                    if score > letter_best_score:
                        letter_best_score = score
                # Usar el mejor score para esta letra
                if letter_best_score > best_score:
                    best_score = letter_best_score
                    best_match = letter
    ...
    def _compare_features(self, feat1: Dict, feat2: Dict) -> float:
        ...
        if feat1['orb_des'] is not None and feat2['orb_des'] is not None:
            try:
                ...
                score += min(1.0, orb_score) * 25.0  # Reducido de 40 a 25
            except:
                pass

        # Histograma
        hist_corr = cv2.compareHist(feat1['histogram'], feat2['histogram'], cv2.HISTCMP_CORREL)
        score += max(0, hist_corr) * 25.0  # Reducido de 30 a 25
        ...
        score += result[0][0] * 35.0  # Aumentado de 15 a 35
        ...
        if 'contour_features' in feat1 and 'contour_features' in feat2:
            ...
            score += contour_similarity * 5.0  # Peso reducido

        # ESCUDO - Feature crítico para diferenciar losetas similares
        shield_match = feat1['has_shield'] == feat2['has_shield']
        if shield_match:
            score += 15.0  # Bonus moderado por escudo matching
        else:
            score -= 12.0  # Penalización moderada por escudo mismatch
