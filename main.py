"""
Módulo principal para procesar tableros de Carcassonne.
Proporciona funciones para analizar imágenes de tableros y calcular puntajes.
"""
import os
from datetime import datetime
from typing import Dict, Tuple, Optional, Union
from modules.piplineFotoRecorteMeepleTipo.origin_matrix_converter import (
    OriginMatrixConverter
)
...
def process_board_image(
    ...
    *,
...
) -> Union[Tuple[Dict[int, int], str, object], Tuple[dict, None, None]]:
    """
    ...
    Args:
        ...
        tiles_texture_pack: Nombre del pack de texturas a usar
    ...
    Returns:
        ...
        O una tupla (dict_confirmacion, None, None) si necesita confirmaciones
    """
    ...
    gen_images.generate_board_image(board_game, output_path)

    # Calcular puntos
    scores_incomplete = GameScorer(board_game).score()
    scores_fields = calculate_field_scores(output_path)

    player1_fields = scores_fields.get(1, 0)
    player1_features = scores_incomplete.get(1, 0)
    player2_fields = scores_fields.get(2, 0)
    player2_features = scores_incomplete.get(2, 0)
    ...
    return scores, output_path, board_game


def run_game():
    """
    Ejecuta el juego procesando un tablero y mostrando los puntajes.
    """
    scores, _, _ = process_board_image("tablero.jpg")

    print(f"jugador 1 = {scores[1]}\njugador 2 = {scores[2]}")
